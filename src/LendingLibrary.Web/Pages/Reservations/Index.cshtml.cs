...
namespace LendingLibrary.Web.Pages.Reservations;

[Authorize(Policy = "RequireUser")]
public class IndexModel(IReservationService reservationService, UserManager<ApplicationUser> userManager) : PageModel
{
    ...
    public async Task OnGetAsync()
    {
        ...
        Results = await reservationService.GetUserReservationsAsync(userId, PageNumber, PageSize);
    }

    public async Task<IActionResult> OnPostCancelAsync(Guid reservationId)
    {
        var userId = Guid.Parse(userManager.GetUserId(User)!);
        var result = await reservationService.CancelAsync(reservationId, userId);
